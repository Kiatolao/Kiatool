...
const galleryContainer = document.querySelector(".gallery");
    ...
    summonButton.addEventListener("click", summonRandomCards);


    function summonRandomCards() {
  ...
  for (let i = 0; i < cardCount; i++) {
    ...
    if (random < rarityProbabilities.legendary) {
      rarity = "legendary";
    } else if (random < rarityProbabilities.legendary + rarityProbabilities.epic) {
      rarity = "epic";
    } else {
      rarity = "rare";
    }

    const shipsOfRarity = hangar.filter(ship => ship.rarity.toLowerCase() === rarity);
    const randomIndex = Math.floor(Math.random() * shipsOfRarity.length);
    randomCards.push(shipsOfRarity[randomIndex]);
  }
